...
export const Orders = () => {
    ...
    if (confirm("¿Quieres solicitar la devolución del producto?")) {
      try {
        const resp = await fetch(
          `http://localhost:8080/compras/${id}/devolver`,
          {
            method: "PUT",
          }
        );
        if (!resp.ok) throw new Error("Error al devolver la compra");
        ...
        console.log(err);
        alert("No se pudo realizar la devuelta");
      }
    }
  };
  ...
  return (
    <div className="ordersContainer">
      {purchases.length == 0 && (
        <div> Aún no has comprado ningún producto </div>
      )}
      {[...purchases]
        ...
        .map((product, index) => (
          <div className="ordersContainer__card" key={`${product.id}-${index}`}>
            <div className="ordersContainer__row--date">
              <div className="ordersContainer__col--date">
                {product.fechaCompra.split("T")[0]}
              </div>
              <hr />
            </div>
            <div className="ordersContainer__row">
              ...
              <div className="ordersContainer__col ordersContainer__col--dev">
                ...
                <button onClick={() => requestReturn(product.compraId)}>
